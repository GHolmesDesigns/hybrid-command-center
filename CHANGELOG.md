...
- Added a committed Cloud Agent development environment (Node 24 toolchain, dependency install, and local database initialization) so the project boots ready to run in one step.
...
- Agent health, presence, summaries, and notifications are now available through MCP tools.
...
- Bumped `googleapis` from 176.0.0 to 178.0.0.
...
- The Tasks page's Pomodoro timer no longer silently reassigns a running session to a
  different task when the timed task leaves the active list (completed, deleted, or moved
  elsewhere). The session now stops and resets to a fresh work phase instead of continuing
  the countdown relabeled under whichever task fills in next.
...
- Buffer now refuses Drive media before submission and explains how to complete the post through a
  supported route.
...
- MCP credential failures now identify authentication problems and document the required
  credential reissue after `SESSION_SECRET` rotation.
...
- Reordered Settings into independent Categories and Tags columns on the left and Defaults, Branding, Drive, Timezones, Calendar, and Campaigns on the right; the Agents navigation and ownership remain unchanged.
