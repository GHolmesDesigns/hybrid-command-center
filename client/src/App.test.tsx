...
const respondTo = (url: string, init?: RequestInit) => {
  ...
  if (method === 'PATCH' && /\/api\/tasks\/[^/]+$/.test(url)) {
    if (taskPatchError) return reply(500, { error: taskPatchError });
    const id = url.split('/api/tasks/')[1];
    tasksPayload = tasksPayload.map((current) => {
    ...
    return tasksPayload.find((current) => current.id === id) ?? {};
  }
...
describe('Dashboard stat tiles', () => {
  ...
  const renderDashboard = async () => {
    dashboardPayload = { ...emptyDashboard, counts };
    render(
      <MemoryRouter initialEntries={['/']}>
        <App />
      </MemoryRouter>,
  ...
  it('navigates on click, and is a real anchor so Enter and Tab work', async () => {
    tasksPayload = [task('tile-week', 'Recap draft', { dueDate: day(3) })];
    await renderDashboard();
...
describe('Task detail inline editing', () => {
  ...
  it('shows empty states that open editors for description, dates, and notes', async () => {
    tasksPayload = [task('t1', 'Recap post')];
  ...
  it('restores the previous value on Cancel and Escape without persisting', async () => {
    tasksPayload = [task('t1', 'Recap post', { description: 'Keep this', notes: 'Leave me' })];
    await openTask('Recap post');
