...
- Posts created: 0; deleted: 0.
- Provider assets created: 0; deleted: 0.
- Independent inventory proof: **not-verified**. No session has run, so there was nothing to delete and nothing to prove.
- Leftovers: none.
...
| --- | --- | --- | --- |
| `POST /v1/posts` accepts `account_configurations` and stores a different caption for each of two explicitly approved accounts on one platform. | **still unverified** | Not attempted in this run. | C77 stays blocked. Today's fail-closed value is unchanged. |
| Which encoding `account_configurations` takes — a list of objects each carrying `account_id`, or a map keyed by account id. | **still unverified** | Not attempted in this run. | C77 stays blocked. Today's fail-closed value is unchanged. |
| The per-account `caption` and `media` read back unchanged through `GET /v1/posts/{id}`, after create and again after `PATCH`. | **still unverified** | Not attempted in this run. | C77 stays blocked. Today's fail-closed value is unchanged. |
| What the provider does about two accounts on one platform when the captions are materially different, and in what terms it states any restriction. | **still unverified** | Not attempted in this run. | C77 stays blocked. Today's fail-closed value is unchanged. |

### Question 2 — Upload flow and media lifecycle

| Claim | State | Evidence | Effect on the dependent cards |
| --- | --- | --- | --- |
| The `create-upload-url` request and response field names, and the signed `PUT`’s required headers and content-length behaviour. | **still unverified** | Not attempted in this run. | C75 stays blocked. Today's fail-closed value is unchanged. |
| That the five documented `mime_type` values are the whole accepted set, and what the provider answers for a value outside it. | **still unverified** | Not attempted in this run. | C74, C75 stay blocked. Today's fail-closed value is unchanged. |
| What `GET /v1/media/{id}` returns for an uploaded asset, and what `describe` says about media a post carries by id rather than by URL. | **still unverified** | Not attempted in this run. | C75 stays blocked. Today's fail-closed value is unchanged. |
| That `media` wins and `media_urls` is ignored when one create request carries both. | **still unverified** | Not attempted in this run. | C75 stays blocked. Today's fail-closed value is unchanged. |
| Whether `DELETE /v1/media/{id}` removes an uploaded asset that is attached to nothing. | **still unverified** | Not attempted in this run. | C75 stays blocked. Today's fail-closed value is unchanged. |
| The 24-hour unattached expiry and the deletion-on-publish the vendor documents. **A single session cannot answer this**: it needs a dated follow-up read of the inventoried asset ids. | **still unverified** | Not attempted in this run. | C75 stays blocked. Today's fail-closed value is unchanged. |
| The current per-file size, item-count, and video-duration limits that apply to the connected plan through the API. | **still unverified** | Not attempted in this run. | C74, C75 stay blocked. Today's fail-closed value is unchanged. |

### Question 3 — Media roles

| Claim | State | Evidence | Effect on the dependent cards |
| --- | --- | --- | --- |
| Whether a scheduled post accepts YouTube `thumbnail` as a provider media id and reads it back — against current support guidance saying custom external thumbnails are unavailable. | **still unverified** | Not attempted in this run. | C76, C82 stay blocked. Today's fail-closed value is unchanged. |
| Whether a scheduled post accepts Instagram `cover_image` as a provider media id and reads it back. | **still unverified** | Not attempted in this run. | C76 stays blocked. Today's fail-closed value is unchanged. |
| That an `application/pdf` asset plus the already-sent `document_title` reads back as a LinkedIn document post. | **still unverified** | Not attempted in this run. | C76 stays blocked. Today's fail-closed value is unchanged. |

### Question 4 — `GET /v1/posts`

| Claim | State | Evidence | Effect on the dependent cards |
| --- | --- | --- | --- |
| The complete pagination contract of `GET /v1/posts` — what `meta.next` holds, and how the last page is recognised. | **still unverified** | Not attempted in this run. | C78 stays blocked. Today's fail-closed value is unchanged. |
| Which repeatable encoding of `status` and `platform` the endpoint actually filters on, `name[]` or a bare repeated `name`. | **still unverified** | Not attempted in this run. | C78 stays blocked. Today's fail-closed value is unchanged. |
| The fields that identify a listed post stably across pages and across reads. | **still unverified** | Not attempted in this run. | C78 stays blocked. Today's fail-closed value is unchanged. |
| That a deleted post is absent from a complete inventory afterwards, rather than present in some other state. | **still unverified** | Not attempted in this run. | C78 stays blocked. Today's fail-closed value is unchanged. |
| The shape of a post created in the provider’s own UI rather than by this app. **A named human precondition**: the owner creates one and passes its id; the probe never pretends to have made it. | **still unverified** | Not attempted in this run. | C78 stays blocked. Today's fail-closed value is unchanged. |

### Question 5 — Analytics

| Claim | State | Evidence | Effect on the dependent cards |
| --- | --- | --- | --- |
| The pagination contract of `GET /v1/analytics`, and whether it matches the posts list. | **still unverified** | Not attempted in this run. | C80 stays blocked. Today's fail-closed value is unchanged. |
| Whether `timeframe` selects which posts are included or which measurement days are counted, and which window values the endpoint accepts. | **still unverified** | Not attempted in this run. | C80 stays blocked. Today's fail-closed value is unchanged. |
| That a filtered response is one row per measured delivery rather than an account aggregate, and that every row still carries `post_result_id`. | **still unverified** | Not attempted in this run. | C80 stays blocked. Today's fail-closed value is unchanged. |
| How a row maps to a social account — through `post_result_id` and the local publication targets, or through some field of its own. | **still unverified** | Not attempted in this run. | C80 stays blocked. Today's fail-closed value is unchanged. |
| The actual values `match_confidence` takes, and whether a record can arrive without one. | **still unverified** | Not attempted in this run. | C79 stays blocked. Today's fail-closed value is unchanged. |

### Question 6 — Platform fields

| Claim | State | Evidence | Effect on the dependent cards |
| --- | --- | --- | --- |
| That YouTube `contains_synthetic_media` is accepted on a scheduled post and reads back with the value that was sent. | **still unverified** | Not attempted in this run. | C81 stays blocked. Today's fail-closed value is unchanged. |
| That TikTok `disclose_branded_content` and `disclose_your_brand` are accepted and read back, with `false` distinguishable from unset. | **still unverified** | Not attempted in this run. | C81 stays blocked. Today's fail-closed value is unchanged. |
| That the already-shipped generic `placement: "story"` path is still accepted for Facebook. | **still unverified** | Not attempted in this run. | C81 stays blocked. Today's fail-closed value is unchanged. |

### Question 7 — Rate-limit evidence

| Claim | State | Evidence | Effect on the dependent cards |
| --- | --- | --- | --- |
| The headers a `429` carries, **if one arrives without being provoked**. The probe has a fixed request budget and never creates load to discover a limit. | **still unverified** | Not attempted in this run. | C75, C80 stay blocked. Today's fail-closed value is unchanged. |
